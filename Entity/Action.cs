...
static public class Action
{
    ...
    static public bool BumpAction(Actor actor, Vector2 direction)
    {
        ...
        if (target)  //If target is within range, we melee the targe. If it's not in range, we will move towards it.
        {
    ...
    static public void MovementAction(Actor actor, Vector2 direction)  //For moving around
    {                                                                  //Updates the field of view and ends turn after
    ...
    {
        int damage = actor.GetComponent<Fighter>().Power() - target.GetComponent<Fighter>().Defense();  //Damage is calculated by subtracting the target's defense from the attacker's power

        string attackDesc = $"{actor.name} attacks {target.name}";

        string colorHex = "";

        if (actor.GetComponent<Player>())     //Changes the color based on whether actor is a player or AI
        {
    ...
    {
        for (int i = 0; i < GameManager.instance.Entities.Count; i++) //Items are types of entities so we loop through all the entities 
        {
            ...
            if (actor.Inventory.Items.Count >= actor.Inventory.Capacity)    //Checks if inventory has anymore space
            {
                UIManager.instance.AddMessage($"Your inventory is full.", "#808080");
                return;
            }

            Item item = GameManager.instance.Entities[i].GetComponent<Item>();      //Gets the item component of the current entity and adds it to the inventory
    ...
    {
        if (actor.Equipment.ItemIsEquipped(item))  //Checks if the them is equipped
        {
            actor.Equipment.ToggleEquip(item);     //If its equipped, we make it unequipped
        }

        actor.Inventory.Drop(item);                //Drop the item

        UIManager.instance.ToggleDropMenu();
        GameManager.instance.EndTurn();
    }

    static public void UseAction(Actor consumer, Item item)     //Allows player to use items
    {
        bool itemUsed = false;
    ...
    {
        if (item.Equippable is null)                        //Checks if item is able to be equipped
        {
            UIManager.instance.AddMessage($"The {item.name} cannot be equipped.", "#808080");
            return;
        }

        actor.Equipment.ToggleEquip(item);                  //Equips item

        UIManager.instance.ToggleInventory();
        GameManager.instance.EndTurn();
    }

    static public void CastAction(Actor consumer, Actor target, Consumable consumable)      //Checks and see if the case is successful, then ends turn 
    {
