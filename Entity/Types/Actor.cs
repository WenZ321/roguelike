...
public class Actor : Entity
{
    ...
    public override void AddToGameManager()
    {
        ...
        if (GetComponent<Player>())                     //Checking if actor is player or not
        {
    ...
    public void LoadState(ActorState state)
    {
        ...
        if (state.CurrentAI != null)                        //If it has an AI
        {
            if (state.CurrentAI.Type == "HostileEnemy")     //It is either a hostile enemy 
            {
                aI = GetComponent<HostileEnemy>();
            }
            else if (state.CurrentAI.Type == "ConfusedEnemy")   //Or confused enemy
            {
...
[System.Serializable]
public class ActorState : EntityState       //Entities that are actors 
{
