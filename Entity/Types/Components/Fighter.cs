...
public class Fighter : MonoBehaviour
{
    ...
    public void Die()
    {
        ...
        if (!GetComponent<Player>())
        {
            GameManager.instance.RemoveActor(this.GetComponent<Actor>());       //Self explanatory 
        }
    }

    public int Heal(int amount)
    {
        if (hp == maxHp)        //Nothing happens if hp is already max
        {
            return 0;
        }

        int newHPValue = hp + amount;   //Self explanatory
