...
sealed class Player : MonoBehaviour, Controls.IPlayerActions
{
    ...
    {
        if (context.started && GetComponent<Actor>().IsAlive)       //Checks if character is alive 
        {
    ...
    public void OnInventory(InputAction.CallbackContext context)    //Self explanantory 
    {
        if (context.performed)
        {
    ...
    public void OnDrop(InputAction.CallbackContext context)        //Same as OnInventory 
    {
        if (context.performed)
        {
        ...
        if (context.performed)
        {
            if (targetMode)
            {
                if (isSingleTarget)
                {
                    ...
                    if (target != null)                                                                                 //If something is there 
                    {
                        Action.CastAction(GetComponent<Actor>(), target, GetComponent<Inventory>().SelectedConsumable); //We cast the action 
                    }
                }
                else
                {
                    ...
                    if (targets != null)                                                                                //Sees if there is any targets 
                    {
                        Action.CastAction(GetComponent<Actor>(), targets, GetComponent<Inventory>().SelectedConsumable); //Then cast action 
                    }
                }
            }
            else if (CanAct())  
    ...
    public void ToggleTargetMode(bool isArea = false, int radius = 1)       //Toggles target mode
    {
        ...
        if (targetMode)
        {
            ...
            if (isArea)                                                    //Checks if it is single or multiple targets
            {
    ...
    private bool CanAct()
    {
        if (targetMode || UIManager.instance.IsMenuOpen || !GetComponent<Actor>().IsAlive)  //If any of these are met, then the character can not act 
        {
            return false;
        }
        else
    ...
    {
        Actor target = GameManager.instance.GetActorAtLocation(targetPosition); //Gets whatever is at the targetPosition
                                                                                //Rest is self explanantory
        if (target == null)
        {
    ...
    private List<Actor> AreaTargetChecks(Vector3 targetPosition)
    {
        ...
        int radius = (int)targetObject.transform.GetChild(0).localScale.x - 1;          //Creates radius around the targetPosition

        Bounds targetBounds = new Bounds(targetPosition, Vector3.one * radius * 2);     //Creates the bounds using radius 
        List<Actor> targets = new List<Actor>();

        foreach (Actor target in GameManager.instance.Actors)                           //Loops through all the characters
        {
            if (targetBounds.Contains(target.transform.position))                       //If character is within the bounds, add it to the target list
            {
                targets.Add(target);
            }
        }

        if (targets.Count == 0)                                                         //If there is no target
        {
