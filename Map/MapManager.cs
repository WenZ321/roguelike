...
public class MapManager : MonoBehaviour
{
    ...
    private void Start()    //Moves camera
    {
    ...
    public GameObject CreateEntity(string entity, Vector2 position)     //Creates entity at specific location
    {
    ...
    public bool IsValidPosition(Vector3 futurePosition)         //Checks if the position you are trying to move to is valid 
    {
        Vector3Int gridPosition = floorMap.WorldToCell(futurePosition);
        if (!InBounds(gridPosition.x, gridPosition.y) || obstacleMap.HasTile(gridPosition)) //Checks by seeing if character is in bounds or if the there is a tile on the obstacle map
        {
            return false;
        }
        return true;
    ...
    {
        foreach (Vector3Int pos in tilemap.cellBounds.allPositionsWithin)       //Loops thru all tiles 
        {
            if (!tilemap.HasTile(pos))  //If there is already a tile there, it will return false 
            {
                continue;               //If there is not a tile it will add it to the tile dictionary 
            }
    ...
    {
        foreach (Vector3Int pos in tiles.Keys)  //Loops through each tiles in the tile dictionary 
        {
            if (!fogMap.HasTile(pos))           //If there is not a fogTile there, return true
            {
                fogMap.SetTile(pos, fogTile);   //Sets of a fogTile at that position
                fogMap.SetTileFlags(pos, TileFlags.None);
            }

            if (tiles[pos].IsExplored)          //Sees if the tile is explored already 
            {
                fogMap.SetColor(pos, new Color(1.0f, 1.0f, 1.0f, 0.5f));
            }
            else
    ...
    public void LoadState(MapState mapState)
    {
        ...
        if (visibleTiles.Count > 0)     //Makes every tiles not visisble 
        {
            visibleTiles.Clear();
        }

        foreach (Vector3Int pos in tiles.Keys)      //Sets up all the tiles by looping thru all of them 
        {
