...
sealed class ProcGen
{
    ...
    private bool SetWallTileIfEmpty(Vector3Int pos) //Name is self explanatory
    {
    ...
    {
        if (MapManager.instance.ObstacleMap.GetTile(pos))       //Removes wall tile if there is one 
        {
    ...
    private void PlaceEntities(RectangularRoom newRoom, int floorNumber)
    {
        ...
        foreach (string entityName in entityNames)                                  //Creates entities at random positions  
        {
